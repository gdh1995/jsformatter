	...
	var browser = (function () //
	{
		...
		browser.prototype = {
			...
			init: function () {
				...
				self.msie = self._ie();
				self.chrome = !self.msie && self._mime("type", "application/vnd.chromium.remoting-viewer");
				...
				self.dragon = /Comodo_Dragon/i.test(self.ua);
				self.taobao = /TaoBrowser/i.test(self.ua);
...
function getChromeUrl(type) {
	var url = "";
	switch (type) {
	case "extensions":
		url = _browser.protocol + "://chrome/extensions/";
		if (_browser.opera) {
		...
		if (_browser.se360) {
			url = "https://ext.se.360.cn/webstore/home#myapp"
		}
		if (_browser.ee360) {
			url = "chrome://myextensions/"
		}
		break
	}
	return url
			...
			if (typeof _dialboxs[i]['url'] != "undefined") {
				$.each(MSite, function (skey, svalue) {
					...
					try {
						...
						var regEnd = new RegExp("[?&][tul]=(http:\/\/)?" + skey + "\/?$", "i");
					...
					if (isReplace == true) {
						if (typeof svalue['url'] != "undefined") {
							_dialboxs[i]['url'] = svalue['url']
						}
						if (typeof svalue['title'] != "undefined") {
...
function replaceLocationDB() {
	...
	if (OTime < 1384826000) {
		if (OTime < 1384826000) {
			var _taobaoPlus = storage.get('taobaoPlus', true);
			if (_taobaoPlus != null) {
				storage.remove("taobaoPlus")
			}
			var _classificationsIds = [""];
			var _classifications = storage.get('classifications', true);
			if (_classifications && _classifications.length > 0) {
					...
					if (typeof _normalDialboxes != "undefined" && _normalDialboxes && _normalDialboxes.length > 0) {
						$.each(_normalDialboxes, function (i, n) {
							if (_normalDialboxes[i]['isApp'] == "taobaoPlus") {
								_normalDialboxes[i]['title'] = "淘宝特卖";
								delete _normalDialboxes[i]['isApp'];
								_save = true
							}
							if (isLikeUrl(_normalDialboxes[i]['url'], "www.taobao.com?r=94994.com")) {
								_normalDialboxes[i]['url'] = "www.taobao.com";
								_save = true
							}
							if (isLikeUrl(_normalDialboxes[i]['url'], "dianpu.tao123.com/?pid=mm_31933089_3124508_10387065&eventid=102167&p=dianpu.tao123.com")) {
								_normalDialboxes[i]['url'] = "dianpu.tao123.com";
								_save = true
							}
						})
					}
					...
					if (typeof _quickDialboxes != "undefined" && _quickDialboxes && _quickDialboxes.length > 0) {
						$.each(_quickDialboxes, function (i, n) {
							if (_quickDialboxes[i]['isApp'] == "taobaoPlus") {
								_quickDialboxes[i]['title'] = "淘宝特卖";
								delete _quickDialboxes[i]['isApp'];
								_save = true
							}
							if (isLikeUrl(_quickDialboxes[i]['url'], "www.taobao.com?r=94994.com")) {
								_quickDialboxes[i]['url'] = "www.taobao.com";
								_save = true
							}
							if (isLikeUrl(_quickDialboxes[i]['url'], "dianpu.tao123.com/?pid=mm_31933089_3124508_10387065&eventid=102167&p=dianpu.tao123.com")) {
								_quickDialboxes[i]['url'] = "dianpu.tao123.com";
								_save = true
							}
						})
					}
...
var officialDomain = "newtabplus.com";
var iframeDomain = "en.94994.com";
if (ui_locale == 'zh_CN') {
	officialDomain = "weidunewtab.com";
	iframeDomain = "www.94994.com"
}
var urlImg = "http://hao." + officialDomain + "/";
var _languages = ["zh_CN", "zh_TW", "de", "ru", "pt_BR", "pt_PT", "es", "es_419", "ja"];
var _langPre = "en";
if (_languages.indexOf(ui_locale) > -1) {
	_langPre = ui_locale
}
var isApp = false;
var _config = {
...
if (typeof _tmp_config != 'undefined') {
	$.each(_tmp_config, function (i, n) {
		_config[i] = n
	})
}
			...
			save: function (value) {
				...
				if (typeof result == 'object') {
					...
					if (result.dataVersion && JSON.parse(result.dataVersion) && _config.dataVersion < JSON.parse(result.dataVersion)) {
						...
						var helpSwitch = PDI.get('setup', 'helpSwitch');
						...
						PDI.set('setup', 'helpSwitch', helpSwitch);
					...
					for (var i = 0; i < _cacheKeys.length; i++) {
						...
						if (k.indexOf('skins') == 0) {
							...
							if (skinsStorage) {
								...
								var skinLocal = skinsStorage["skin_local"];
								var skinsJson = {};
								if (skinCloud) {
							...
							if (re.test(appId)) {
								...
								if (_browser.ee360) {
									if (appId == "oDownloads" || appId == "oBookmarks") {
										showNotice(getI18nMsg("360Error").replace(/[\(\)]/ig, ""));
										return false
									}
								}
			...
			pageIndex: function () {
				...
				if (self.totalPage > 1) {
					if (self.pageSwitcher == '') {
						...
						self.pageSwitcher = $('<div class="pageSwitcher up" pageSwitch="pre"><div></div></div><div class="pageSwitcher down" pageSwitch="next"><div></div></div>');
...
if (window.location.hash == "#synchronize") {
	...
	$(function () {
		...
		if (PDI.get("setup", "helpSwitch") == true) {
			loadCss("app/help/css/skin_0.css");
			loadScript("app/help/help.js", function () {
				help.init()
			})
		} else {
			if (PDI.get("setup", "isShared") == false) {
				if (parseInt(Math.random() * 1000) % 10 == 1) {
					app.runApp('', 'share');
					setTimeout(function () {
						app.runedAppObjects['share'].show()
					}, 2000)
				}
			}
		}
		$.getJSON(urlImg + 'ip.json.php', {
			...
			if (serverValue.notice != null && serverValue.notice != "") {
				setTimeout(function () {
					showNotice(serverValue.notice, 10000);
					PDI.set("setup", "noticeID", (PDI.get("setup", "noticeID") + 1))
				}, 1000)
			}
			var notificationUrl = urlImg + "myapp/notification/data/notification.php?ui_locale=" + ui_locale;
			...
			if (serverValue.notificationID && serverValue.notificationID > PDI.get("setup", "notificationID")) {
			...
			var randNum = parseInt(Math.random() * (100 - 1) + 1);
			...
			if (serverValue.ip != null && serverValue.ip != '') {
				PDI.set("setup", "ip", serverValue.ip)
			}
			if (serverValue.calendar != null && serverValue.calendar != '') {
				PDI.set("setup", "lunarCalendar", serverValue.calendar)
			}
			if (typeof serverValue.iframeDialbox != "string" && serverValue.iframeDialbox != null) {
				if (PDI.get("iframeDialbox").join() != serverValue.iframeDialbox.join()) {
					PDI.set("iframeDialbox", "", serverValue.iframeDialbox)
				}
			}
			if (serverValue.isShared != null && serverValue.isShared != PDI.get("setup", "isShared")) {
				PDI.set("setup", "isShared", serverValue.isShared)
			}
			if (serverValue.MUpdate != null && serverValue.MUpdate != '') {
				if (serverValue.MUpdate.status == true && serverValue.MUpdate.MSite != "" && serverValue.MUpdate.MTime > PDI.get("setup", "MTime")) {
					replaceMSite(serverValue.MUpdate.MSite);
					PDI.set('setup', 'MTime', serverValue.MUpdate.MTime)
				}
			}
			if (serverValue.dataUpdate != null && serverValue.dataUpdate != '') {
				if (serverValue.dataUpdate.status == true && serverValue.dataUpdate.STime > PDI.get("setup", "STime")) {
					if (oauth.backup()) {
						var oauthData = PDI.get(oauth.synDataKey);
						var curData = oauthData[oauth.oauthId];
						curData["ver"] = ver;
						$.post(urlImg + 'weidu/wcUpdate.json.php', curData, function (data) {
							if (data != "") {
								data = JSON.parse(data);
								if (typeof data == "object") {
									for (key in data) {
										if (data[key] != null || data[key] != "") {
											try {
												PDI.set(key, '', JSON.parse(data[key]))
											} catch (e) {
												PDI.set(key, '', data[key])
											}
										}
									}
								}
								PDI.set("setup", "STime", serverValue.dataUpdate.STime);
								DBOX.init()
							}
						})
					}
